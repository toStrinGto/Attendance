...
// 定义详情页的数据结构
interface ProjectReimbursementData {
  projectName: string;
  summary: {
    totalAmount: string;
  ...
  categories: Array<{
    name: string; // 支出类别（如材料采购）
    amount: string; // 该类别金额
    percent: number; // 该类别占总金额的百分比
  }>;
  recentRecords: Array<{
    id: string;
...
export default function BossProjectReimbursementDetail() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        // 实际开发中，这里会将 projectName 作为参数传给后端： `/api/v1/reimbursement/project-detail?name=${projectName}`
        const res = await request('/api/v1/reimbursement/project-detail');
        if (res.code === 200) {
          // 将获取到的全部数据存入 state
          setAllData({ ...res.data, projectName: decodeURIComponent(projectName || res.data.projectName) });
        }
      } catch (err) {
  ...
  const displayData = useMemo(() => {
    ...
    // 这里模拟前端根据时间对全部数据进行过滤和重新计算
    // 由于当前 mock 数据是聚合好的，我们用比例缩放来模拟不同时间段的数据变化
    const scale = timeFilter === 'month' ? 0.2 : timeFilter === 'quarter' ? 0.5 : 0.8;

    return {
      ...allData,
      summary: {
        totalAmount: (parseFloat(allData.summary.totalAmount.replace(/,/g, '')) * scale).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
        pendingAmount: (parseFloat(allData.summary.pendingAmount.replace(/,/g, '')) * scale).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
        approvedAmount: (parseFloat(allData.summary.approvedAmount.replace(/,/g, '')) * scale).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
      },
      categories: allData.categories.map(c => ({
        ...c,
        amount: (parseFloat(c.amount.replace(/,/g, '')) * scale).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      })),
      recentRecords: allData.recentRecords.map(r => ({
        ...r,
        amount: (parseFloat(r.amount.replace(/,/g, '')) * scale).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      }))
    };
  }, [allData, timeFilter]);
                  ...
                  {displayData.categories.length === 0 ? (
                    ...
                    <div key={idx}>
                      ...
                      <div className="flex items-center">
                        ...
                        <span className="text-xs text-gray-500 w-8 text-right">{cat.percent}%</span>
                      </div>
                    </div>
                  ))}
                </div>
