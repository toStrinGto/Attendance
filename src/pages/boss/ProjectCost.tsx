...
export default function BossProjectCost() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Add mock dailyWage to workers if not present
        const workersWithWage = workersRes.data.map((w: Worker) => ({
          ...w,
          dailyWage: w.dailyWage || Math.floor(Math.random() * 200 + 200) // 200-400
        }));
  ...
  // Generate mock data when project changes
  useEffect(() => {
    if (!selectedProjectId) return;
    
    // Mock attendance records for the selected project
    const mockAttendance: AttendanceRecord[] = [];
    workers.forEach(w => {
      // Randomly assign some workers to this project
      if (Math.random() > 0.3) {
        const days = Math.floor(Math.random() * 20 + 5); // 5-25 days
        for (let i = 0; i < days; i++) {
          mockAttendance.push({
            workerId: w.id,
            date: `2024-03-${String(i + 1).padStart(2, '0')}`,
            dayShift: 1,
            overtimeHours: Math.floor(Math.random() * 4) // 0-3 hours
          });
        }
      }
    });
    ...
    // Mock reimbursements
    const mockReimbursements: ReimbursementRecord[] = Array.from({ length: Math.floor(Math.random() * 5 + 2) }).map((_, i) => ({
      id: i,
      amount: Math.floor(Math.random() * 5000 + 500),
      description: ['材料费', '交通费', '餐补', '住宿费'][Math.floor(Math.random() * 4)],
