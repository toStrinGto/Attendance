...
// 定义后端返回的数据结构（TypeScript 接口），这有助于代码提示和错误检查
interface ReimbursementOverviewData {
  summary: {
  ...
  projects: Array<{
    id: string;
    name: string; // 项目名称
    totalAmount: string; // 该项目报销总额
    pendingCount: number; // 该项目待审批数
    approvedCount: number; // 该项目已通过数
...
export default function BossReimbursementOverview() {
  ...
  const displayData = useMemo(() => {
    ...
    // 实际开发中，如果后端返回的是明细列表，前端可以根据日期真实过滤
    ...
    return {
      ...
      projects: allData.projects.map(p => ({
        ...p,
        totalAmount: (parseFloat(p.totalAmount.replace(/,/g, '')) * scale).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
        pendingCount: Math.ceil(p.pendingCount * scale),
        approvedCount: Math.ceil(p.approvedCount * scale),
      }))
    };
  }, [allData, timeFilter]);
