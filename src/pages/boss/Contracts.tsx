...
export default function Contracts() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modalMode === 'add') {
        const newContract = { ...formData, id: `${targetType === 'income' ? 'INC' : 'EXP'}-${Date.now()}` } as Contract;
        ...
        setData(prev => ({
          ...prev,
          [targetType]: [newContract, ...prev[targetType]]
        }));
        ...
        await request(`/api/v1/contracts/${selectedContract.id}`, {
          method: 'PUT',
        ...
        setData(prev => {
          ...
          if (originalType === targetType) {
            newData[targetType] = newData[targetType].map(c => c.id === selectedContract.id ? { ...c, ...updatedContract } : c);
          } else {
            newData[originalType] = newData[originalType].filter(c => c.id !== selectedContract.id);
            newData[targetType] = [{ ...updatedContract, id: selectedContract.id }, ...newData[targetType]];
          }
          return newData;
        });
      }
