...
export default function IncomeSettlementPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modalMode === 'add') {
        const newItem = { ...payload, id: `INC-SET-${Date.now()}` };
        await request('/api/v1/income-settlements', {
          method: 'POST',
          body: JSON.stringify(newItem)
        });
        setData(prev => [newItem, ...prev]);
      } else if (modalMode === 'edit' && selectedItem) {
        await request(`/api/v1/income-settlements/${selectedItem.id}`, {
          method: 'PUT',
          body: JSON.stringify(payload)
        });
        setData(prev => prev.map(item => item.id === selectedItem.id ? { ...item, ...payload } : item));
      }
      handleCloseModal();
    } catch (err) {
