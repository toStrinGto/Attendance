...
export default function BossEmployeeDetail() {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      setLoading(true);
      try {
        const json = await request(`/api/v1/employees/${id || '1'}/attendance`);
        
        const baseData = json.data;
        const generated = Array.from({ length: 500 }).map((_, i) => {
          ...
          return {
            ...
            time: template.status === 'present' ? `07:5${Math.floor(Math.random() * 9)}` : null,
