...
export default function DailyReport() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        if (json.data.length > 0) {
          ...
          setReportContent(json.data[0].content);
        }
      } catch (err) {
    ...
    // Add a small delay to show the skeleton animation clearly
    setTimeout(fetchInitialData, 600);
  }, []);
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      if (editingTemplate) {
        await request(`/api/v1/reports/templates/${editingTemplate.id}`, {
          method: 'PUT',
          body: JSON.stringify({ name: tempName, content: tempContent })
        });
        const updatedTemplates = templates.map(t => 
          t.id === editingTemplate.id ? { ...t, name: tempName, content: tempContent } : t
        );
        setTemplates(updatedTemplates);
        
        // Update selected template if it's the one being edited
        if (selectedTemplate?.id === editingTemplate.id) {
          const updatedSelected = { ...editingTemplate, name: tempName, content: tempContent };
          setSelectedTemplate(updatedSelected);
          if (reportContent === editingTemplate.content) {
            setReportContent(tempContent);
          }
        }
      } else {
        await request('/api/v1/reports/templates', {
          method: 'POST',
          body: JSON.stringify({ name: tempName, content: tempContent })
        });
        const newTemplate = {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Simulate refreshing history by prepending a new record
      const newRecord = {
        id: Date.now(),
                ...
                <div key={report.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className={cn(
                      ...
                      report.status === '已阅' ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-blue-50 text-blue-600 border-blue-100"
                    )}>
                      {report.status}
                    </span>
                  </div>
          ...
          <div className="absolute inset-0 z-[100] flex items-center justify-center p-4 bg-black/50">
            ...
            >
              <div className="p-4 border-b border-gray-100 flex justify-between items-center">
                ...
                <button onClick={() => setIsModalOpen(false)} className="text-gray-400 active:text-gray-600">
                  <X className="w-5 h-5" />
                </button>
              </div>
