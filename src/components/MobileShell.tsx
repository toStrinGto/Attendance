...
export default function MobileShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 w-full">
      {/* Content Area */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar pb-16">
        {children}
      </div>

      {/* Bottom Navigation */}
      <div className="fixed bottom-0 w-full h-16 bg-white border-t border-gray-200 flex justify-around items-center px-2 pb-safe z-50">
        {navItems.map((item) => {
